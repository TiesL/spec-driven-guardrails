...
output="$(PATH="$fakebin:$PATH" "$script" zonder-pr 2>&1)"; status=$?
[ "$status" -ne 0 ] || fail "S52 — a commit without a PR gave exit 0"
assert_contains "S52 — the message mentions the commit" "zonder-pr" "$output"

# S52 — a merge commit that does come from a PR is let through.
output="$(PATH="$fakebin:$PATH" "$script" met-pr 2>&1)"; status=$?
...
output="$(PATH="$fakebin:$PATH" "$script" onbekend 2>&1)"; status=$?
...
output="$(PATH="$path_without_gh" "$script" zonder-pr 2>&1)"; status=$?
