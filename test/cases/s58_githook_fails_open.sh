...
project="$(fresh_project broken-source)"
...
output="$(cd "$project" && git commit -q --allow-empty -m "tweede commit op main" 2>&1)"
status=$?
...
assert_contains "S58 — a warning appears" "warning" "$output"
