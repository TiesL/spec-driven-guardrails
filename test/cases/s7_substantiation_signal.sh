...
# Given: a freshly adopted project with 22 seeded rows carrying "requires
# substantiation".
project="$(fresh_project target-project)"
adopt "$project"

rows="$(grep -c 'requires substantiation' "$project/WORKFLOW-ADOPTION.md")"
[ "$rows" -eq 22 ] || fail "S7 — $rows rows with 'requires substantiation', 22 expected"
...
# And the count moves along: substantiating one row makes it twenty-one.
