...
# Then: exact 21 rijen, allemaal met "vereist onderbouwing".
rijen="$(grep -c '^| [a-z]' "$tabel")"
[ "$rijen" -eq 21 ] || fail "R1 — $rijen rijen geseed, 21 verwacht"

onderbouwing="$(grep -c 'vereist onderbouwing' "$tabel")"
[ "$onderbouwing" -eq 21 ] || fail "R1 — $onderbouwing rijen met 'vereist onderbouwing', 21 verwacht"
