...
allowed "semicolon in a message"  'git commit -m "note; git clean -fd is bad"'
allowed "ampersand in a message"  'git commit -m "recipe & git reset --hard combo"'
allowed "pipe in an echo"            'echo "see README | git reset --hard is in there"'
allowed "multi-line message"      'git commit -m "a line
git reset --hard is here as text"'
...
allowed "heredoc with git in the body" 'cat <<EOF >> README.md
git reset --hard
EOF'
allowed "heredoc with quoted delimiter" "cat <<'EOF' >> doc.md
...
# command.
allowed "delimiter as a word in the body" 'cat <<EOF > x
this is not EOF but ordinary text
git reset --hard
EOF'
allowed "heredoc with tabs (<<-)"     "$(printf 'cat <<-EOF > x\n\tgit clean -fd\n\tEOF\n')"

# But a real command after a heredoc is judged normally.
blocked "command after a heredoc"   'cat <<EOF > x
text
EOF
git reset --hard'

# And quotes around a flag belong to the command, not to data.
blocked "quoted flag"               'git reset "--hard"'
blocked "quoted dot"               'git checkout -- "."'
blocked "backslash escape"           'git reset \-\-hard'

# An imbalance in the quoting is not a command we can read; do not guess then.
allowed "quoting that does not close"      'git commit -m "open'
