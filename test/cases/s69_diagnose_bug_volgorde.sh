...
assert_contains "S69 — noemt regressietest" "egressietest" "$inhoud"
...
pos_repro="$(grep -n '^\*\*1\. Reproductie' "$skill" | head -1 | cut -d: -f1)"
pos_hyp="$(grep -n '^\*\*2\. Hypotheses' "$skill" | head -1 | cut -d: -f1)"
pos_regr="$(grep -n '^\*\*3\. Regressietest' "$skill" | head -1 | cut -d: -f1)"
...
assert_contains "S69 — hypotheses worden getoond vóór ze getest worden" "getoond" "$inhoud"
