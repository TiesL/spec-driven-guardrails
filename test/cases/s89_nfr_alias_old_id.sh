...
project="$(fresh_project alias-old-id)"
...
if grep -qx 'spec-data-integrity' "$actual"; then
...
output="$SANDBOX/scope-output.txt"
"$TEST_REPO_ROOT/skills/pre-merge-review/scope.sh" "$project" "$TEST_REPO_ROOT" > "$output" 2>/dev/null

if ! grep -qx 'spec-data-integriteit: Data integrity' "$output"; then
