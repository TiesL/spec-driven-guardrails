...
Executes `spec-driven-guardrails`' adoption steps using the answers collected in F3, narrating each step's purpose and outcome as it happens. **Steps, corrected 2026-09-26 to match the two-plugin architecture and the copies-everywhere propagation decision** (see "Build order decided" and "The propagation decision" in `ARCHITECTURE.md` — this replaces `adopt.sh`'s symlink-based step list, not a copy of it): install `spec-driven-guardrails-workflow` at local scope (carries the skills and hooks — no separate "install skill files" or "symlink" step, since the plugin mechanism itself is the delivery, not a project-local symlink); copy `CLAUDE.md` and `.claude/settings.json`'s attribution setting into the target repo (copies, not symlinks); write the managed `.gitignore` block; seed the adoption table. **Implemented directly in v1, primitives extracted later (decided 2026-09-26):** rather than building the full execute/plan/verify primitive apparatus before a real step list exists, F4 is implemented directly against this step list first; extracting it into the tested primitive set (`ARCHITECTURE.md`) is later work (see Epics), once real steps exist to generalize from — the same "de-risk before generalizing" principle `ARCHITECTURE.md`'s "Build order decided" already applies to the plugin split, applied one level deeper. Cross-platform-safe (`ARCHITECTURE.md` A5) regardless of when the extraction happens — not a wrapper that shells out to `adopt.sh`'s existing Bash-only script body. **Not parity with `adopt.sh`** (F7's verification criterion below states this explicitly): plugin-carried skills are a genuine scope widening, and copies instead of symlinks are a genuine propagation-model change — the enumerated expected-tree check in F7 lists these as intended deviations, not defects.
...
| **E3 (F9, re-adoption) is unfunded, but it is the only way a workflow change reaches an adopted project once symlinks retire.** Under `adopt.sh`'s old symlink model, propagation was automatic; under copies (decided 2026-09-26), a project stays frozen at whatever it was copied from until F9 runs. This affects Ties' own four projects, not only future adopters, and it's what `adopt.sh`'s own retirement trigger depends on. | E2 alone (funded) is still a real, working, one-shot adoption — this debt is about staying current afterward, not about E2 shipping something broken | Before `adopt.sh` is actually retired (its stated trigger already requires F9 to exist); revisit funding E3 sooner if this bites Ties' own projects first |
